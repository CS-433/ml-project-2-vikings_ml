...
class UNet():
    ...
    def res(self, depth, kernelsize=(3,3)):
        ...
        model = keras.Sequential([
                    Conv2D(2**depth * self.filter_num, kernelsize, activation='relu', padding=self.pad),
                    BatchNormalization(),
    ...
    def encoder_ladder(self, input, depth, func, add=[], first=False):
        ...
        x = Conv2D(2**depth * self.filter_num, (3,3), padding=self.pad, kernel_initializer='he_normal')(input)
        print(f"depth: {depth}")
    ...
    def decoder_ladder(self, input, saved_layer, depth, func):
        ...
        # 2x2 convolution (“up-convolution”), upsampling
        up_conv = Conv2DTranspose(2**depth * self.filter_num, (3,3), strides=2, padding="same")(input)
    ...
    def get_model_ladder(self, image_shape):
        """
        Creates the Ladder-net architecture (consisting of two U-nets) for a picture with a given input size
        """
        input = keras.Input(shape=image_shape)
        ...
        saved_layer0, left0 = self.encoder_ladder(input, 0, shared_resnets[0], first=True)
        saved_layer1, left1 = self.encoder_ladder(left0, 1, shared_resnets[1], first=True)
        saved_layer2, left2 = self.encoder_ladder(left1, 2, shared_resnets[2], first=True)
        saved_layer3, left3 = self.encoder_ladder(left2, 3, shared_resnets[3], first=True)

        # Convolution between contracting and expansive path
        middle = Conv2D(2**4 * self.filter_num, (3,3), padding=self.pad, kernel_initializer='he_normal')(left3)
        ...
        # Expansive path (right side) of the network architecture
        right3 = self.decoder_ladder(middle, saved_layer3, 3, shared_resnets[3])
        right2 = self.decoder_ladder(right3, saved_layer2, 2, shared_resnets[2])
        right1 = self.decoder_ladder(right2, saved_layer1, 1, shared_resnets[1])
        right0 = self.decoder_ladder(right1, saved_layer0, 0, shared_resnets[0])
        ...
        saved_layer1, left1 = self.encoder_ladder(left0, 1, shared_resnets[1], right1)
        saved_layer2, left2 = self.encoder_ladder(left1, 2, shared_resnets[2], right2)
        saved_layer3, left3 = self.encoder_ladder(left2, 3, shared_resnets[3], right3)
